...
def remove_redundant_v4(path, node_path, tolerance=1e-3):
    assert path
    new_path = [path[0]]
    new_node_path = [node_path[0]]

    for conf, conf_node in zip(path[1:], node_path[1:]):
        difference = get_difference(new_path[-1], np.array(conf))
        if not np.allclose(np.zeros(len(difference)), difference, atol=tolerance, rtol=0):
            new_path.append(conf)
            new_node_path.append(conf_node)

    return new_path, new_node_path


def waypoints_from_path_v4(path, node_path, tolerance=1e-3):
    path, node_path = remove_redundant_v4(path, node_path, tolerance=tolerance)

    if len(path) < 2:
        return path, node_path

    waypoints = [path[0]]
    waypoints_nodes = [node_path[0]]

    last_conf = path[1]
    last_conf_node = node_path[1]

    last_difference = get_unit_vector(get_difference(last_conf, waypoints[-1]))

    for conf, conf_node in zip(path[2:], node_path[2:]):
        difference = get_unit_vector(get_difference(conf, waypoints[-1]))

        if not np.allclose(last_difference, difference, atol=tolerance, rtol=0):
            waypoints.append(last_conf)
            waypoints_nodes.append(last_conf_node)
            difference = get_unit_vector(get_difference(conf, waypoints[-1]))

        last_conf = conf
        last_conf_node = conf_node
        last_difference = difference

    waypoints.append(last_conf)
    waypoints_nodes.append(last_conf_node)

    return waypoints, waypoints_nodes
