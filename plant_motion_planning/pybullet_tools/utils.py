...
def get_model_path(rel_path): # TODO: add to search path
    directory = os.path.dirname(os.path.abspath(__file__))
    # return os.path.join(directory, '../plant_motion_planning', rel_path)
...
def get_contact_points_with_force(force_limit, **kwargs):
    # return [CollisionInfo(*info) for info in p.getContactPoints(physicsClientId=CLIENT, **kwargs)]

    # contact_points = p.getContactPoints(physicsClientId=CLIENT, **kwargs)
    contact_points = get_contact_points(**kwargs)

    for cp in contact_points:
        if(cp.normalForce > force_limit):
            return True

    return False

def pairwise_link_contact(body1, link1, body2, link2=BASE_LINK, force_limit = 0, **kwargs):
    return get_contact_points_with_force(force_limit = force_limit, bodyA = body1, bodyB = body2, linkIndexA=link1, linkIndexB=link2)

def any_link_pair_contact(body1, links1, body2, links2=None, force_limit = 0, **kwargs):
    # TODO: this likely isn't needed anymore
    if links1 is None:
        links1 = get_all_links(body1)
    if links2 is None:
    ...
    for link1, link2 in product(links1, links2):
        if (body1 == body2) and (link1 == link2):
            continue
        if pairwise_link_contact(body1, link1, body2, link2, force_limit, **kwargs):
            return True
    return False

link_pairs_contact = any_link_pair_contact

def body_contact(body1, body2, force_limit, **kwargs):
    return get_contact_points_with_force(force_limit = force_limit, bodyA = body1, bodyB = body2)

def pairwise_contact(body1, body2, force_limit, **kwargs):
    if isinstance(body1, tuple) or isinstance(body2, tuple):

        body1, links1 = expand_links(body1)
        body2, links2 = expand_links(body2)
        return any_link_pair_contact(body1, links1, body2, links2, force_limit, **kwargs)
    return body_contact(body1, body2, force_limit, **kwargs)
...
def get_collision_fn_with_angle_constraints_v4(body, fixed, joints, movable, deflection_limit, attachments=[], self_collisions=True,
                                               disabled_collisions=set(), custom_limits={}, use_aabb=False, cache=False,
                                               max_distance=MAX_DISTANCE, **kwargs):

    is_colliding = get_collision_fn_with_controls_v3(body, fixed, joints, attachments, self_collisions,
                                                     disabled_collisions, custom_limits, use_aabb, cache, max_distance,
                                                     **kwargs)
    ...
    def collision_fn(q, verbose=False):
        ...
        for e, b in enumerate(movable):
            ...
            if(b.observe_all_and_check(deflection_limit) and np.random.rand() < 0.95):
                print("=======================")
            ...
            if (verbose):
                print("Deflection of plant in collision fn", b.deflections)
...
def get_collision_fn_with_controls_v3(body, fixed, joints, attachments=[], self_collisions=True, disabled_collisions=set(),
                                      custom_limits={}, use_aabb=False, cache=False, max_distance=MAX_DISTANCE, **kwargs):
    # TODO: convert most of these to keyword arguments
    check_link_pairs = get_self_link_pairs(body, joints, disabled_collisions) if self_collisions else []
    moving_links = frozenset(link for link in get_moving_links(body, joints)
    ...
    def collision_fn(q, verbose=False):
        ...
        for body1, body2 in product(moving_bodies, fixed):
            if (not use_aabb or aabb_overlap(get_moving_aabb(body1), get_obstacle_aabb(body2))) \
                    and pairwise_collision(body1, body2, **kwargs):
                #print(get_body_name(body1), get_body_name(body2))
                if verbose: print(body1, body2)
...
def get_collision_fn_with_controls_benchmark_multiworld(body, joints, obstacles=[], attachments=[], self_collisions=True, disabled_collisions=set(),
                                      custom_limits={}, use_aabb=False, cache=False, max_distance=MAX_DISTANCE, **kwargs):
    # TODO: convert most of these to keyword arguments
    check_link_pairs = get_self_link_pairs(body, joints, disabled_collisions) if self_collisions else []
    moving_links = frozenset(link for link in get_moving_links(body, joints)
...
def get_collision_fn2(body, joints, obstacles=[], attachments=[], movable = [], force_limit = 0, self_collisions=True, disabled_collisions=set(),
                     custom_limits={}, use_aabb=False, cache=False, max_distance=MAX_DISTANCE, **kwargs):
    # TODO: convert most of these to keyword arguments
    check_link_pairs = get_self_link_pairs(body, joints, disabled_collisions) if self_collisions else []
    moving_links = frozenset(link for link in get_moving_links(body, joints)
    ...
    def collision_fn(q, verbose=False):
        ...
        for link1, link2 in check_link_pairs:
            ...
            # TODO: self-collisions between body and attached_bodies (except for the link adjacent to the robot)
            if (not use_aabb or aabb_overlap(get_moving_aabb(body), get_moving_aabb(body))) and \
                    pairwise_link_contact(body, link1, body, link2, force_limit): #, **kwargs):
                #print(get_body_name(body), get_link_name(body, link1), get_link_name(body, link2))
                if verbose: print(body, link1, body, link2)
                return True
        ...
        for b in movable:
            ...
            if (not use_aabb or aabb_overlap(get_moving_aabb(body), get_obstacle_aabb(b))) \
                    and pairwise_contact(body, b, force_limit, **kwargs):


                contact_points = get_contact_points(bodyA = body, bodyB = b)
                # if(len(contact_points) > 0):
                for cp in contact_points:
                    if(len(cp) > 0 and cp.normalForce > force_limit):
                        print("normal force exceeded: ", cp.normalForce)
...
def plan_joint_motion_with_angle_contraints(body, joints, end_conf, obstacles=[], attachments=[],
    ...
    collision_fn = get_collision_fn_with_angle_constraints(body, joints, obstacles, movable, deflection_limit, attachments, self_collisions, disabled_collisions,
                                                           custom_limits=custom_limits, max_distance=max_distance,
                                                           use_aabb=use_aabb, cache=cache)
...
def plan_joint_motion_with_angle_constraints_v5(body, start_state_id, joints, end_conf, obstacles=[], attachments=[],
                                               movable = [], deflection_limit = 0, self_collisions=True, disabled_collisions=set(),
                                               weights=None, resolutions=None, max_distance=MAX_DISTANCE,
                                               use_aabb=False, cache=True, custom_limits={}, algorithm=None, **kwargs):
    ...
    distance_fn = get_distance_fn(body, joints, weights=weights)
    extend_fn = get_extend_fn(body, joints, resolutions=resolutions)
    collision_fn = get_collision_fn_with_angle_constraints_v3(body, joints, obstacles, movable, deflection_limit, attachments, self_collisions, disabled_collisions,
                                                              custom_limits=custom_limits, max_distance=max_distance,
                                                              use_aabb=use_aabb, cache=cache)
...
def plan_joint_motion2(body, joints, end_conf, obstacles=[], movable = [],
    ...
    distance_fn = get_distance_fn(body, joints, weights=weights)
    extend_fn = get_extend_fn(body, joints, resolutions=resolutions)
    collision_fn = get_collision_fn2(body, joints, obstacles, attachments, movable, force_limit, self_collisions, disabled_collisions,
                                    custom_limits=custom_limits, max_distance=max_distance,
                                    use_aabb=use_aabb, cache=cache)
...
def plan_lazy_prm(start_conf, end_conf, sample_fn, extend_fn, collision_fn, **kwargs):
    ...
    from plant_motion_planning.motion_planners import lazy_prm
