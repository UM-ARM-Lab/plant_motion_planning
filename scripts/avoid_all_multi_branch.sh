#!/bin/bash
# Execute cfg.ROOT_DIR/scripts/avoid_all_multi_branch.py with the appropriate arguments for many trials
...
#  python -m scripts.move_kuka_avoid_all_with_controls --deflection_limit "$3" --video_filename "$2"/env"$counter"/trial1.mp4
  python -m scripts.avoid_all_multi_branch --deflection_limit "$3" --video_filename "$2"/trial"$counter".mp4 --plant_filename "$4"  >> "$2"/trial"$counter".txt
