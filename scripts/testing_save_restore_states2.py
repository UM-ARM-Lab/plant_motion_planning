...
goal_conf1 = (1.5, -1.0, 1.0, 1.5,0.0,0.0,0.0)
goal_conf2 = (1.8, -1.0, 1.0, 1.5,0.0,0.0,0.0)
...
def main(display='execute'):  # control | execute | step
    ...
    conf_g1 = BodyConf(robot, configuration=goal_conf1)
    conf_g2 = BodyConf(robot, configuration=goal_conf2)

    # Moving arm from conf_i to conf_g
    command1 = move_arm_conf2conf(robot,[floor],conf_i, conf_g1)
    command2 = move_arm_conf2conf(robot,[floor],conf_g1, conf_g2)

    if (command1 is None) or (display is None):
    ...
    if display == 'control':
        enable_gravity()
        command1.control(real_time=False, dt=0)
    elif display == 'execute':
        command1.refine(num_steps=10).execute(time_step=0.005)
    elif display == 'execute_with_movable_plant':
        command1.refine(num_steps=10).execute_with_movable_plant(robot, [plant1, plant2], time_step=0.005)
        plant1_2angle_rep.observe()
        plant2_2angle_rep.observe()
        print(f"deflection of plant1: {plant1_2angle_rep.deflection} | plant2: {plant2_2angle_rep.deflection}")
        # saved_state_goal1 = p.saveState()
        saved_state_goal1 = WorldSaver()
    elif display == 'step':
        command1.step()
    else:
    ...
    # p.restoreState(saved_state_goal1)
    # saved_world.restore()
    saved_state_goal1.restore()
    update_state()
    wait_if_gui('{}?'.format(display))
    if display == 'control':
        enable_gravity()
        command2.control(real_time=False, dt=0)
    elif display == 'execute':
        command2.refine(num_steps=10).execute(time_step=0.005)
    elif display == 'execute_with_movable_plant':
        command2.refine(num_steps=10).execute_with_movable_plant(robot, [plant1, plant2], time_step=0.005)
        plant1_2angle_rep.observe()
        plant2_2angle_rep.observe()
        print(f"deflection of plant1: {plant1_2angle_rep.deflection} | plant2: {plant2_2angle_rep.deflection}")
        # saved_state_goal2 = p.saveState()
        saved_state_goal2 = WorldSaver()
    elif display == 'step':
        command2.step()
    else:
        raise ValueError(display)
    ...
    print("go back to goal2 state?")
    wait_if_gui()
    # p.restoreState(saved_state_goal2)
    saved_state_goal2.restore()
    plant1_2angle_rep.observe()
    plant2_2angle_rep.observe()
    print(f"deflection of plant1: {plant1_2angle_rep.deflection} | plant2: {plant2_2angle_rep.deflection}")
    # saved_state_init.restore()
