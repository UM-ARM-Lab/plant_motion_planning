...
from plant_motion_planning.pybullet_tools.kuka_primitives import BodyConf, Command, get_free_motion_gen, \
    get_free_motion_gen_with_controls, get_free_motion_gen_single_plant
from plant_motion_planning.pybullet_tools.utils import enable_gravity, connect, dump_world, set_pose, \
    ...
    update_state, \
    disable_real_time, HideOutput, DRAKE_IIWA_URDF_EDIT, get_movable_joints, get_num_joints
import pybullet as p
from plant_motion_planning.utils import step_sim, \
    load_plant_from_urdf, generate_random_plant, generate_tall_plants, envs
...
def move_arm_conf2conf(robot, fixed, conf_i, conf_g):
    ...
    free_motion_fn = get_free_motion_gen_single_plant(robot, fixed=fixed, teleport=False)
