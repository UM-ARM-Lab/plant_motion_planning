...
def main(display='execute'): # control | execute | step
    ...
    input("Starting with the initial configuration...")

    p.setJointMotorControlArray(robot, get_movable_joints(robot), p.POSITION_CONTROL, init_conf,
                                positionGains=7*[0.01])
    step_sim()

    input("Moving to the goal configuration...")

    p.setJointMotorControlArray(robot, get_movable_joints(robot), p.POSITION_CONTROL, goal_conf,
                                positionGains=7 * [0.01])
    step_sim()

    input("Resetting back to initial configuration...")
    set_joint_positions(robot, get_movable_joints(robot), init_conf)
    p.setJointMotorControlArray(robot, get_movable_joints(robot), p.POSITION_CONTROL, init_conf,
                                positionGains=7*[0.01])
    step_sim()
    # step_sim()
