...
def move_arm_conf2conf2(robot, fixed, movable, deflection_limit, conf_i, conf_g, teleport=False):
...
def main(display='execute'): # control | execute | step
    ...
    # set_random_poses([plant1, plant2], floor)
    ...
    set_random_pose(plant1, floor, px = -1.0, py = -1.0)
    ...
    # command = move_arm_conf2conf(robot, [floor, block], [plant2_2angle_rep, plant1_2angle_rep, plant3_2angle_rep, plant4_2angle_rep],
    #                              deflection_limit, conf_i, conf_g)
    command = move_arm_conf2conf(robot, [floor, block], [plant1_2angle_rep],
                                 deflection_limit, conf_i, conf_g)
    ...
    log_id = p.startStateLogging(loggingType=p.STATE_LOGGING_VIDEO_MP4,
                    fileName="../simulation_recordings/angle_constraint_v2/rec23_unedited.mp4")
    if display == 'control':
        enable_gravity()
        ...
        command.execute_with_movable_plant_angle_constraint(robot, block, [plant1],
                                                            [plant1_2angle_rep], deflection_limit, cost_utils)
    else:
