...
from .utils import set_random_poses, make_plant_responsive, set_random_pose, step_sim
...
def move_arm_conf2conf(robot, fixed, movable, deflection_limit, conf_i, conf_g, teleport=False):
    ...
    free_motion_fn = get_free_motion_gen_with_angle_constraints_v2(robot, start_state_id, fixed= fixed, movable = movable,
...
goal_conf2 = (-1.6871757013351371, 1.6063773991870438, 2.152853076950719, -1.0638334445672613, -0.1398096715085235,
             -1.1079682303638, -1.051507203470595)
...
def main(display='execute'): # control | execute | step
    ...
    # set_random_pose(plant1, floor, px = 0.4, py = 0.1)
    # set_random_pose(plant2, floor, px = 0.22, py = -0.35)
    ...
    # command = move_arm_conf2conf(robot, [floor, block], [plant1_2angle_rep, plant2_2angle_rep,
    ...
    #                 fileName="../simulation_recordings/angle_constraint_v3/rec1_unedited.mp4")
    # if display == 'control':
    #     enable_gravity()
    ...
    # set_joint_positions(robot, joints, goal_conf2)
    pos_gains = 7 * [0.01]
    # vel_gains = 7 * [0.001]
    # p.setJointMotorControlArray(robot, joints, p.POSITION_CONTROL, goal_conf, positionGains=pos_gains, velocityGains=vel_gains)
    p.setJointMotorControlArray(robot, joints, p.POSITION_CONTROL, goal_conf, positionGains=pos_gains)
    ...
    print("robot in initial config")
    input("Press enter to move to next config")

    t = 0
    while(t < 20):

        current_joint_state = np.array(p.getJointStates(robot, joints))[:,0]
        print(np.linalg.norm(current_joint_state - np.array(goal_conf)))

        print("current joint state: ", current_joint_state)
        print("++++++++++++++++++++++++++++++++++++++++++")
        print("goal config: ", goal_conf)
        print("=================================")
        # input("")
