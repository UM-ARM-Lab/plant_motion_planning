...
from pybullet_tools.kuka_primitives import BodyPose, BodyConf, Command, get_grasp_gen, \
    ...
    get_free_motion_gen_with_angle_constraints_v3
...
def move_arm_conf2conf(robot, fixed, movable, deflection_limit, conf_i, conf_g, teleport=False):
    ...
    free_motion_fn = get_free_motion_gen_with_angle_constraints_v3(robot, start_state_id, fixed= fixed,
                                                                   movable = movable,
...
def main(display='execute'): # control | execute | step
    ...
    log_id = p.startStateLogging(loggingType=p.STATE_LOGGING_VIDEO_MP4,
                    fileName="../simulation_recordings/angle_constraint_v3/rec3_unedited.mp4")
    if display == 'control':
        enable_gravity()
