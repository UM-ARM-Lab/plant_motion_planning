...
def move_arm2(robot, block, force_limit, fixed, movable, teleport):
    ...
    ## PROBLEMATIC
    # free_motion_fn = get_free_motion_gen(robot, fixed=([block] + fixed), teleport=teleport)
    free_motion_fn = get_free_motion_gen2(robot, fixed=([block] + fixed), movable = movable,
                                          force_limit = force_limit, teleport=teleport)
...
def main(display='execute'): # control | execute | step
    connect(use_gui=True,width=1200, height=400)
    # connect(use_gui=True)
    disable_real_time()

    p.setAdditionalSearchPath(pybullet_data.getDataPath())  # optionally
    # log_id = p.startStateLogging(loggingType=p.STATE_LOGGING_VIDEO_MP4, fileName="../videos/planning_around_movable_plants_full_avoid/rec2.mp4")
    ...
    plant = load_model('urdf/short_plant_multi_dof.urdf', fixed_base=True)
    ...
    p.setJointMotorControlArray(plant, [0, 1], p.VELOCITY_CONTROL, targetVelocities=[0, 0],
    ...
    ## Force limit - applied on the plants by the robot
    force_limit = 10
    ...
    command = move_arm2(robot, block, force_limit = force_limit, fixed=[floor], movable = [plant], teleport=False)
    ...
    if display == 'control':
        ...
        command.refine(num_steps = 10).execute_with_movable_plant(robot, plant,time_step = 0.005)
    elif display == 'step':
